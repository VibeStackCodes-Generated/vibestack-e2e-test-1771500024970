...
export const Route = createFileRoute('/restaurant-tables/$id')({
  component: RestaurantTableDetailPage,
})

function RestaurantTableDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <article className="mx-auto max-w-3xl p-6 gap-6 py-12 space-y-8">
        ...
        <h1 className="text-4xl font-[family-name:var(--font-display)]">{String(row.location ?? 'Restaurant Table')}</h1>
        <section className="space-y-2"><h2 className="text-xl font-[family-name:var(--font-display)]">Details</h2><dl>
              <div className="py-2 border-b last:border-0">
                ...
                <dd className="text-sm mt-1">{String(row.location ?? '—')}</dd>
              </div></dl></section><section className="space-y-2"><h2 className="text-xl font-[family-name:var(--font-display)]">Properties</h2><dl>
              <div className="py-2 border-b last:border-0">
                <dt className="text-xs uppercase tracking-wide text-muted-foreground">Table Number</dt>
                <dd className="text-sm mt-1">{String(row.table_number ?? '—')}</dd>
              </div>
              <div className="py-2 border-b last:border-0">
                <dt className="text-xs uppercase tracking-wide text-muted-foreground">Capacity</dt>
                <dd className="text-sm mt-1">{String(row.capacity ?? '—')}</dd>
              </div>
              <div className="py-2 border-b last:border-0">
                <dt className="text-xs uppercase tracking-wide text-muted-foreground">Status</dt>
                <dd className="text-sm mt-1">{String(row.status ?? '—')}</dd>
              </div></dl></section>
        <Link to="/restaurant-tables/"><Button variant="outline" className="transition-all duration-200">Back to Restaurant Tables</Button></Link>
      </article>
    </div>
  )
}
