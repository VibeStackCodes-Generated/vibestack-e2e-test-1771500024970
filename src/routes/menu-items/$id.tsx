...
export const Route = createFileRoute('/menu-items/$id')({
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <article className="mx-auto max-w-3xl p-6 gap-6 py-12 space-y-8">
        ...
        <h1 className="text-4xl font-[family-name:var(--font-display)]">{String(row.name ?? 'Menu Item')}</h1>
        <section className="space-y-2"><h2 className="text-xl font-[family-name:var(--font-display)]">Details</h2><dl>
              ...
              <div className="py-2 border-b last:border-0">
                ...
                <dd className="text-sm mt-1">{String(row.category ?? '—')}</dd>
              </div>
              <div className="py-2 border-b last:border-0">
                <dt className="text-xs uppercase tracking-wide text-muted-foreground">Price</dt>
                <dd className="text-sm mt-1">{Number(row.price ?? 0).toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</dd>
              </div></dl></section>
        <Link to="/menu-items/"><Button variant="outline" className="transition-all duration-200">Back to Menu Items</Button></Link>
      </article>
    </div>
  )
}
