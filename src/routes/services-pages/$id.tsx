...
export const Route = createFileRoute('/services-pages/$id')({
...
function ServicesPageDetailPage() {
  ...
  const { data: row, isPending, error } = useQuery({
    queryKey: ['services_page', id],
    queryFn: async () => {
      const { data, error } = await supabase.from('services_page').select('*').eq('id', id).single()
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <article className="mx-auto max-w-3xl p-6 gap-6 py-12 space-y-8">
        ...
        <h1 className="text-4xl font-[family-name:var(--font-display)]">{String(row.name ?? 'Services Page')}</h1>
        <section className="space-y-2"><h2 className="text-xl font-[family-name:var(--font-display)]">Details</h2><dl>
              ...
              <div className="py-2 border-b last:border-0">
                ...
                <dd className="text-sm mt-1">{String(row.url ?? '—')}</dd>
              </div>
              <div className="py-2 border-b last:border-0">
              ...
              <div className="py-2 border-b last:border-0">
                <dt className="text-xs uppercase tracking-wide text-muted-foreground">Order Index</dt>
                <dd className="text-sm mt-1">{String(row.order_index ?? '—')}</dd>
              </div></dl></section>
        <Link to="/services-pages/"><Button variant="outline" className="transition-all duration-200">Back to Services Pages</Button></Link>
      </article>
    </div>
  )
}
