  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <article className="mx-auto max-w-3xl p-6 gap-6 py-12 space-y-8">
        ...
        <h1 className="text-4xl font-[family-name:var(--font-display)]">{String(row.name ?? 'Reservation')}</h1>
        <section className="space-y-2"><h2 className="text-xl font-[family-name:var(--font-display)]">Details</h2><dl>
              ...
              <div className="py-2 border-b last:border-0">
                ...
                <dd className="text-sm mt-1">{String(row.email ?? '—')}</dd>
              </div>
              <div className="py-2 border-b last:border-0">
              ...
              <div className="py-2 border-b last:border-0">
                <dt className="text-xs uppercase tracking-wide text-muted-foreground">Party Size</dt>
                <dd className="text-sm mt-1">{String(row.party_size ?? '—')}</dd>
              </div></dl></section>
        <Link to="/reservations/"><Button variant="outline" className="transition-all duration-200">Back to Reservations</Button></Link>
      </article>
    </div>
  )
}
